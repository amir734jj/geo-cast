import { type FindOneOptions, type FindOptionsOrder, type FindOptionsWhere, type Repository } from 'typeorm'
...
export abstract class AbstractDal<T extends EntityType> implements IBasicCrud<T> {
  ...
  async patch (id: number, partial: Partial<T>): Promise<T | null> {
    await this.repository.update(
      id,
      { ...partial } as QueryDeepPartialEntity<T>
    )
    return await this.repository.findOneBy({ id } as FindOptionsWhere<T>)
  }
}
