...
export default class AuthService {
  ...
  public async register(userData: CreateUserDto): Promise<User> {
    ...
    return await this.userService.save({
      ...
      roles: await Promise.all(
        roles.map(async (role) => await this.getOrCreateRole(role))
      ),
    });
  }
  ...
  public async login(loginUserDto: LoginUserDto): Promise<string | null> {
    ...
    if (
      ...
      await this.userService.save({ ...user, lastLoginAt: DateTime.now().toJSDate() });
  ...
  private async setUserRoles(
    ...
    user.roles = user.roles.concat(
      await Promise.all(
        missingRoles.map(async (role) => await this.getOrCreateRole(role))
      )
    );
