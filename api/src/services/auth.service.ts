...
export default class AuthService {
  ...
  public async register (userData: CreateUserDto): Promise<User> {
    ...
    return await this.userService.save({
      ...
      roles: await Promise.all(roles.map(async (role) => await this.getOrCreateRole(role)))
    })
  }
  ...
  public async logout (user: User): Promise<User | null> {
    await Promise.all(user.tokens.map(async (tk) => await this.tokenService.delete(tk.id)))
  ...
  public async setUserRole (
    ...
    switch (role) {
      case UserRole.ADMIN:
        return await this.setUserRoles(user, BASIC_ROLE, ADMIN_ROLE)
      case UserRole.BASIC:
        return await this.setUserRoles(user, BASIC_ROLE)
    }
  }
  ...
  private async setUserRoles (
    ...
    user.roles = user.roles.concat(
      await Promise.all(missingRoles.map(async (role) => await this.getOrCreateRole(role)))
    )
